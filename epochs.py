import copy
import numpy as np
from find_saccades import find_saccades


class Epoch:
    """
    Includes information on the epoch
    and the saccades in it.

    Arguments:
        - event_prop - dict with keys:
            - "event_idx"
            - "onset"
            - "duration"
            - "series"
            - "image_number"
    You can get event_prop by
    calling instance of ExtractEventInfo class.
    """
...
class ExtractEventInfo:
    """
    Extracts information from annotations
    in signal (from mne) in DataFrame
    """
