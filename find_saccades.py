...
def moving_average(signal, len_single_avrg, overlap=1):
    """
    Calculates the value of the moving average
    and extends it over the entire
    length of a signal segment
    """
    sig_len = len(signal)
...
def find_saccades_single_step_length(sig, Fs, min_diff=25, max_diff=80):
    """
    Finds saccades for one segment of moving average.
    """
    ...
    for i in range(sig.shape[0] // avg_len + 1):
        ...
        if np.any(ma_diff < 0) and np.any(ma_diff > 0):
            ...
            # in the neighborhood of 0.2 s,
            # find where the diff is the highest
            # and choose that point
            t_range = int(0.2 * Fs)
